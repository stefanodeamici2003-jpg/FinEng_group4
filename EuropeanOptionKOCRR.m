function optionPrice = EuropeanOptionKOCRR(F0, K, KO, B, T, sigma, N)
    ...
    % --- 2. Payoff calculation at maturity (Time T, Step N) ---
    ...
    for i = 0:N
        ...
        % Payoff calculation
        if FN<KO
            V(i + 1) = max(FN - K, 0); % Call Payoff
        else
            V(i + 1) = 0; 
        end
    end
    
    % --- 3. Backward Induction (Moving backwards in time) ---
    ...
    for step = N-1 : -1 : 0
        ...
        for i = 0:step
            ...
            % The option value at the current node is the discounted expected value.
            % (Note: the discount is not applied at the single step here because you are
            %  discounting everything at the very end by multiplying by B)
            V(i + 1) = p * V_up + (1 - p) * V_down;
            
        end
    end
    
    % --- 4. Discounting to time zero ---
    optionPrice = B * V(1);
end